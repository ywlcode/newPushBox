...
void BeginPage::loginSuccess()
{
	...
	back_music->setVolume(0.1);   //音量  0~1之间
	back_music->play();
	//按钮
	begin_game = new QPushButton("闯关模式", this);
	begin_game->setGeometry(390, 340, 320, 50);
	begin_game->setStyleSheet(btn_style);
	begin_game->setFont(QFont("幼圆", 14));
	begin_game->show();
	inf_game = new QPushButton("无尽模式", this);
	inf_game->setGeometry(390, 420, 320, 50);
	inf_game->setStyleSheet(btn_style);
	inf_game->setFont(QFont("幼圆", 14));
	inf_game->show();
	leaderboard = new QPushButton("排行榜", this);
	leaderboard->setGeometry(390, 500, 320, 50);
	leaderboard->setStyleSheet(btn_style);
	leaderboard->setFont(QFont("幼圆", 14));
	leaderboard->show();
	game_set = new QPushButton("游戏设置", this);
	game_set->setGeometry(390, 580, 320, 50);
	...
	connect(inf_game, &QPushButton::clicked, this, &BeginPage::infchallenge);
...
void BeginPage::seeLeader()
{
	...
	while (query.next())
	{
		...
		leaders[i]->setText(QString(QString("%1").arg(query.value(0).toString(), -15 - query.value(0).toString().length() / 4, ' ') + QString("%1").arg(query.value(18).toString(), -3, ' ')));
